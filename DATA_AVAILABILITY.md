...
- **Finance-Agent and WorkBench: acquisition is automated.** Run the documented setup scripts (`bash scripts/setup_finance_agent.sh` and `bash scripts/setup_workbench.sh`) once; each shell script clones the upstream repository and invokes its Python conversion adapter to convert upstream task definitions into the normalized JSON consumed by the loader (applying the deterministic stratified 100-instance subsample for WorkBench), and writes the dataset config. Upstream raw data are not redistributed; users obtain them through their upstream sources, mediated by the setup scripts. No manual JSON placement is required.
